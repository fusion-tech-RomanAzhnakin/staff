...
const connectRoute = (io) => {
  ...
  io.on('connection', (socket) => {
    ...
    function isUserRole() {
      const role = _get(socket, 'handshake.role');
      const status = _get(socket, 'handshake.status');
      return ['user', 'mentor'].includes(role) && status === 'active';
    }
    ...
    function addAdminsRoute() {
      ...
      socket.on(socketMessage.CHECK_JOB_URL_ALREADY_IN_USE, async (job_link) => {
        if (!job_link) {
          return;
        }

        const exists = await controllerTasks.isJobLinkExists(job_link);
        socket.emit(socketMessage.CHECK_JOB_URL_ALREADY_IN_USE, exists);
      });
    }

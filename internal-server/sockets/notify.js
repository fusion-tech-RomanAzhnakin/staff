...
const changeData = {
  ...
  invite: 'Изменен статус инвайт',
  hot: 'Изменен статус "горячего" таска',
  project_start_date: 'Изменена дата старта контракта',
  project_end_date: 'Изменена дата завершения контракта',
};
...
function generateTaskUrl(task) {
  ...
  return `${config.crmAddress}/${path}?open=${task.id}`;
}
...
async function sendNotificationForSubscribedAndChannel(message, idAuthor) {
  ...
  try {
    ...
    try {
      const slackMessage = `:rocket: *<${generateTaskUrl(task)}|Fusion CRM message in task:>   \`${task.title}\`*\n_${author.firstName} ${author.lastName} оставил сообщение:_\n${message.message}`;
      const data = {
...
async function slackCRMTaskNotify(user, userAuthor, message, idTask) {
  if (env !== 'development') {
    ...
    try {
      task = await db.crm_tasks.findByPk(idTask);
    ...
    const textMessage = `:rocket: *<${generateTaskUrl(task)}|Fusion CRM message in task:>   \`${taskName}\`*\n${authorName}${message}`;
...
async function crmLogging(task, update, authorId) {
  ...
  try {
    author = await db.user.findByPk(authorId);
  } catch (error) {
  ...
  try {
    ...
    if (env !== 'development') {
      const message = `:rocket: *<${generateTaskUrl(task)}|Fusion CRM message in task:>   \`${task.title}\`*\n${slackMessage}`;
      const data = {
        channel: task.proposal ? config.CRMApplyChannelId : config.CRMChannelId,
        text: message,
      };
...
async function getDifferenceString(oldTask, newTask) {
  ...
  if (oldTask.task_in_column !== newTask.task_in_column) {
    ...
    try {
      const oldColumn = await db.crm_columns.findByPk(oldTask.task_in_column);
      const newColumn = await db.crm_columns.findByPk(newTask.task_in_column);
  ...
  if (newTask.archive !== oldTask.archive) {
    if (newTask.archive) {
      ...
      if (newTask.reject_reason_id) {
        try {
          const reason = await db.crm_reject_reasons.findByPk(newTask.reject_reason_id);

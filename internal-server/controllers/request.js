...
const getRequests = async (req, res) => {
  try {
    ...
    if (query.filter.type && !allowedListOfTypesFor[req.user.role].includes(query.filter.type)) {
      return res.status(403).json({ message: `Not acceptable for role ${req.user.role}` });
    }
    ...
    filter = { ...filter, ...createRequestFilterByRole(query, req.user.role) };
...
const putRequest = async (req, res) => {
  try {
    ...
    if (newData.type === 'vacation') {
      ...
      newData.is_unpaid = req.body.isUnpaid;
    }

    if (newData.type === 'timeOff') {
      newData.type = 'dayOff';
      newData.rest_days_number = 0;
    }

    const request = await db.request.findByPk(newData.id, roleOpportunities);
...
const postRequest = async (req, res) => {
  try {
    ...
    switch (type) {
      ...
      case 'medical':
      case 'vacation':
        payload.is_unpaid = payload.isUnpaid;
    ...
    request = await db.request.findByPk(request.id, {
      include: [
        {
          model: db.user,
          attributes: ['id', 'login', 'firstName', 'lastName', 'role'],
        },
      ],
    });
...
const allowedListOfTypesFor = {
  ...
  hr: ['vacation', 'dayOff', 'medical'],
  manager: ['vacation', 'dayOff', 'medical'],
  officeManager: ['documents', 'technical', 'common'],
};

const createRequestFilterByRole = ({ filter = {} } = {}, role) => {
  if (filter.type) {

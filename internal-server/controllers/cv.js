    ...
    if (size(rolesInProjects) > 0) {
      rolesInProjects.forEach((projectObject, index) => {
        const projectData = find(
          presentProjects,
          project => project.id === projectObject.value
        );
        if (projectData) {
          projectData.role = rolesInProjects[index].role;
          projectData.technologies = rolesInProjects[
            index
          ].technologies.map(tech => ({ title: tech.label }));
        }
      });
    }
    if (presentProjects) {
...
const textCheck = (text = '') => text.trim().replace(/<p><br><\/p>(?=<p><br><\/p>)/gm, '');

...
const postPlan = async (req, res) => {
  try {
    ...
    plan = await db.plan.findByPk(plan.id, {
      include: [
        { model: db.taskJob },
        {
          model: db.user,
...
const putPlan = async (req, res) => {
  try {
    ...
    if (newPlanPayload.editNote) {
      let plan = await db.plan.findByPk(req.params.id);
      plan = await plan.update(newPlanPayload.plan);
      return res.send(plan);
    }

    const {
      jobsToAdd, oldChain, newChain,
    } = newPlanPayload;
    ...
    plan = await db.plan.findByPk(plan.id, { include: { model: db.taskJob } });
...
const calculateHolidaysAndWeekends = async (idUser, idRequest, newDataPayload) => {
  ...
  const oldplanItem = await db.plan_taskJob.findByPk(idRequest);
  ...
  const startTaskYear = moment(startTaskDate).format('YYYY');
  const finishTaskYear = moment(finishTaskDate).format('YYYY');
  const diffStart = startTaskDate.diff(moment(calendarGenerator.calendar.dates[startTaskYear][0].day, 'DD-MM-YYYY'), 'days');
  const diffEnd = (finishTaskDate.diff(startTaskDate, 'days') + 1);

  if (startTaskYear !== finishTaskYear) {
    const fullStartYear = calendarGenerator.calendar.dates[startTaskYear];
    const fullFinishYear = calendarGenerator.calendar.dates[finishTaskYear];
    const years = [...fullStartYear, ...fullFinishYear];
    days = years.slice(diffStart, diffEnd + diffStart);
  } else {
    days = calendarGenerator.calendar.dates[moment(startTaskDate).format('YYYY')].slice(diffStart, diffEnd + diffStart);
  }
...
const putTaskJobInPlan = async (req, res) => {
  try {
    ...
    planItem = await db.plan_taskJob.findByPk(req.params.id);

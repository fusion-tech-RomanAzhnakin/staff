...
class SelectFromDB extends Component {
  ...
  async componentDidUpdate(prevProps) {
    const { isMyData, selectedUserId } = this.props;
    if (
      isMyData !== prevProps.isMyData ||
      selectedUserId !== prevProps.selectedUserId
    ) {
      if (isMyData === 'general') {
        this.props.change();
      } else {
        const userProjects = this.state.options.filter(
          option => option.users.some(user => user.id === selectedUserId)
        );

        this.props.change(userProjects);
      }
    }
  ...
  users: async (selectedFromProps, filter, isMulti) => {
    ...
    for (let i = 0; i < dataFromDB.length; i++) {
      const option = {
        ...
        label_ru: getFullName(dataFromDB[i], 'ru'),
        education: dataFromDB[i].education || '',
        education_ru: dataFromDB[i].education_ru || '',
        value: dataFromDB[i].id
      };
      options.push(option);
      if (
...
SelectFromDB.propTypes = {
  ...
  isMyData: PropTypes.string.isRequired,

...
import { Button, Menu, MenuItem, TextField, Divider, Typography } from '@material-ui/core';
...
class RequestModal extends Component {
  ...
  dateSelector = createSelector(
    [
      ...
      (props) => props.data.rest_days_number
    ],
    (type, date, dates, dateFrom, dateTo, typeOfRequest, restDaysNumber) => {
      return formatDate(type, date, dates, dateFrom, dateTo, typeOfRequest, restDaysNumber);
    }
  );
  ...
  handleDelClick = (event) => {
    this.setState({ anchorDel: event.currentTarget });
  };
      ...
      <StyledModal open={this.props.show} onClose={this.onHide}>
        ...
        <StyledFooter>
          {this.props.type === 'send' ? (
            <>
              <Button onClick={this.props.submit} value="accept" className="accept-btn">
                Отправить заявку
              </Button>

              <Button onClick={this.onHideElement} value="decline" className="decline-btn">
                Отмена
              </Button>
            </>
                  ...
                  {!this.state.edit ? (
                    <>
                      ...
                      <Button
                        ...
                        aria-owns={this.state.anchorDel ? 'del-menu' : undefined}
                        aria-haspopup="true"
                      ...
                      <Menu
                        id="del-menu"
                        anchorEl={this.state.anchorDel}
                        open={Boolean(this.state.anchorDel)}
                        onClose={this.handleDelClose}
                      >
                        <MenuItem className="decline-btn" onClick={this.props.delete}>
                          Удалить заявку
                        </MenuItem>
                        <MenuItem className="decline-btn" onClick={this.handleDelClose}>
                          Отмена
                        </MenuItem>
                      </Menu>
                    </>
...
const formatDate = (type, date, dates, dateFrom, dateTo, typeOfRequest, restDaysNumber) => {
  let newDate = date;
  newDate = {
    dateFrom,
    dateTo,
  ...
  newDate = getDate(newDate, 'server', typeOfRequest, restDaysNumber);
  return newDate;
};

...
export const createHours = (date) => {
  ...
  return `${newdDate.hour() > 9 ? newdDate.hour() : `0${newdDate.hour()}`}:${newdDate.minute() > 9 ? newdDate.minute() : `0${newdDate.minute()}`}`;
};
...
export const getDate = (date, dataOf, type = null, restDays = null) => {
  let str = '';
...
export const getName = (user, language = 'en') => {
  if (user) {
    const {
      firstName = '',
      lastName = '',
    ...
    if (language === 'ru') {
      if (!firstName_ru && !lastName_ru) {
        return login;
      }
      return `${firstName_ru} ${lastName_ru}`;
    }
  }
  return '';
};

export const getUserName = (user, type = 'full', lang = 'ru') => {
  const {
 ...
 * @param {"ru" | "en"} lang
 */
export const getFullNameV2 = (user, lang = 'ru') => {
  const fullName_en = `${user.firstName || ''} ${user.lastName || ''}`.trim();
  const fullName_ru = `${user.firstName_ru || ''} ${user.lastName_ru || ''}`.trim();

  const fullName = lang === 'ru' ? fullName_ru : fullName_en;
  const fullName_alt = lang === 'en' ? fullName_ru : fullName_en;

  return fullName || fullName_alt || user.email;
};

export const getFullName = (user, language) => {
  return getName(user, language);
};

export const getExpandedName = (user) => {
  const nameRu = getName(user, 'ru');
  const nameEn = getName(user, 'en');

  if (nameRu === nameEn && nameRu === user.login) {
    return `${nameRu} (${user.email})`;
  }
...
export const createDataChart = ({
  ...
  const paidVacationDays = (groupStatistics.vacation || []).filter((vacation) => {
    return !vacation.is_unpaid;
  });
  ...
  return [
    ...
    },
    {
      ...
      days: getQuantityDay(paidVacationDays)
    },
    {
      name: 'Документы',
...
export const formatUserToSelect = (user) => (
  {
    value: user.id,
    label: getName(user),
    avatar: user.avatar,
...
export const getAvatar = ({ avatarThumbnail, avatar } = {}) => {

...
class RequestInfo extends Component {
  ...
  cancel = async () => {
    try {
      ...
      this.props.updateEntitiesState();
      this.setState({
        open: false
      });
    } catch (err) {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <StyledPanelHeader>
            <Typography style={{ fontSize: '1.1rem' }}>{request.title}</Typography>
        ...
        <ExpansionPanelDetails>
          <StyledPanel>
            <Typography component="span">
              <b>Когда: </b>
              <pre>{getDate(request.date, 'server', request.type, request.rest_days_number)}</pre>
            </Typography>
            {request.type === 'vacation' && (
              <Typography component="span">
                {`Тип отпуска: ${request.is_unpaid ? 'Неоплачиваемый' : 'Оплачиваемый'}`}
              </Typography>
            )}
            <br />
            {request.comment && (
            ...
            {request.willCompensateHours && (
              <Typography component="span">
                <br />
                <b>Отработаю часы в другой день</b>
              </Typography>
            )}

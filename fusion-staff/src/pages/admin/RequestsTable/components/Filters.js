...
class Filters extends Component {
  ...
  createFilterRequest = createSelector(
    ...
    (role, filter) => {
      return role === 'officeManager' ? filter.slice(3, filter.length)
        : role !== 'admin' ? filter.slice(0, 3) : filter;
    }
  );
  ...
  search = (e) => {
    e.preventDefault();
    const data = {};
    if (this.state.user) { data.from = (this.state.user.value); }
...
const style = {
  ...
  sortButton: { margin: '0 0 10px 20px' },

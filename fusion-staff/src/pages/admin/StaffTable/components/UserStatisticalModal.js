...
import { getFullName, dateToString, createDataChart, getModalStyle } from 'utils';
import { getUserRequestStatisticRequest } from 'api/userRequestApi';

import { Typography, Paper, Modal, Button, Divider } from '@material-ui/core';
import { DateRange, CustomActiveShapePieChart as CustomPieChart } from 'ui';
...
class UserStatisticalModal extends Component {
  ...
  getStatistic = async (dates) => {
    try {
      const { data: userRequests } = await getUserRequestStatisticRequest(this.props.user.id, {
        dates,
        status: 'completed'
      });
      ...
      >
        <StyledPaper style={getModalStyle()}>
          ...
          {pickedRange.value === 'dateRange' && !dates && (
            <DateRange userRole={globalUser.role} allDates={true} sendRequest={this.datePick} />
          )}
          <br />
          ...
          <StyledDivider variant="inset" />
          <Typography gutterBottom noWrap>
            <b style={{ display: 'block', padding: '10px 0' }}>Дни отсутвования: </b>
          </Typography>
          {dataChart && (
            <>
              <Typography gutterBottom noWrap>
              ...
              <Typography gutterBottom noWrap>
                ...
                {extraHours
                  ? ` ${extraHours} ${
                    extraHours === 1 ? 'час' : /2|3|4/.test(`${extraHours}`) ? 'часа' : 'часов'
                  } `
                  : '0 часов'}
              </Typography>
            </>

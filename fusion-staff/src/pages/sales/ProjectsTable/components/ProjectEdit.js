...
import { getModalStyle, getValueFromSelect } from 'utils';
import { getImagesChange, changeCheck, getIdArray } from 'utilsImages';

import { Typography, Paper, Modal, Button, Fab, TextField } from '@material-ui/core';
import { SelectFromDB, RichTextBox } from 'ui';
import ImagesDownload from 'pages/sales/CreateProject/components/ImagesDownload';

class ProjectEdit extends Component {
  constructor(props) {
    super(props);
    const { title, href, description, images, role, description_ru } = props.project;
  ...
  applyEdit = async () => {
    const data = {
      ...this.state,
      description_ru: this.state.descriptionRu,
  ...
  onDescriptionRuChange = (descriptionRu) => {
    this.setState({
      descriptionRu
    });
  };
      ...
      <StyledModal onClose={this.hide} open={this.state.show}>
        <StyledPaper style={getModalStyle()}>
          <StyledForm>
            ...
            <div className="edit-project__input-field">
              <RichTextBox value={descriptionRu} onChange={this.onDescriptionRuChange} />
            </div>
            <Typography gutterBottom noWrap>
              <b>Роли:</b>
            </Typography>
            {role.map((el, index) => {
              const { text, title } = el;
              return (
                <div className="edit-project__input-field relative" key={`roleItem ${index}`}>
                  <RichTextBox
                    value={text}
                    roleTitle={title}
            ...
            <Fab color="secondary" aria-label="add" className="fab-add" onClick={this.addRole}>
              <AddIcon />
            </Fab>
            <div className="edit-project__input-field">
              <ImagesDownload
            ...
            <div className="edit-project__input-field">
              ...
              <label htmlFor="project-techonologies" className="edit-project__input-label">
                Технологии:
              </label>
              <SelectFromDB
            ...
            <div className="edit-project__input-field">
              {/* eslint-disable-next-line */}
              <label htmlFor="project-devs" className="edit-project__input-label">
                Разработчики:
              </label>
              <SelectFromDB
                id="project-devs"
